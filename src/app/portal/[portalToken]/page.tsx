...
type PortalDoc = {
  ...
  status: string;
  branch: { name: string; phone: string; email: string };
  items: PortalItem[];
...
export default async function PortalPage({
  ...
  let data;
  try {
    data = await getPortalDocument(portalToken);
  } catch {
    return (
  ...
  if (data.type === 'QUOTATION') {
    const doc = data.document as unknown as PortalDoc;
    return (
        ...
        <div>
          ...
          <div className="flow-root">
            <ul role="list" className="-mb-8">
              {doc.logs?.map((log: PortalLog, logIdx: number) => (
                <li key={log.id}>
                  <div className="relative pb-8">
                    {logIdx !== (doc.logs?.length ?? 0) - 1 ? (
                      <span
    ...
    const totalPaid = doc.payments!.reduce(
      (acc: number, p: PortalPayment) => acc + Number(p.amount),
      0
    );
